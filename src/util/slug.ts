...
export const urlsafeSlugProcessor = (string: string) =>
  slugify(string, {
    customReplacements: [
...
const isNonAlphaNumUnicode = (string) =>
  !alphaNumRegex.test(string) && unicodeRegex.test(string);

export const unicodeSafeProcessor = (string: string) =>
  string
    .split("")
    .reduce(
...
export const makeSlugProcessor = (preserveUnicode: boolean) =>
  preserveUnicode ? unicodeSafeProcessor : urlsafeSlugProcessor;

/* session processor */

export const makeSessionSlugProcessor = (preserveUnicode: boolean) => {
